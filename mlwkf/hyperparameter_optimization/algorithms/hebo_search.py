...
def hyperband_search_algorithm(training_dataset, oos_dataset, selected_features, model_function, hyper_parameters, n_iteration, scoring_functions, scoring_function_to_use_for_evaluation, output_folder, cpus_per_job, gpu_per_job):
    ...
    hyperbandopt = TuneBOHB(max_concurrent=20, metric=scoring_function_to_use_for_evaluation, mode="max")

    hyperbandshd = HyperBandForBOHB(
        max_t=100,
    ...
    analysis = tune.run(
        objective_function,
        search_alg=hyperbandopt,
        scheduler=hyperbandshd,
...
def run_hyperband_search_algorithm(training_dataset, oos_dataset, selected_features, model_function, hyper_parameters, scoring_functions, n_iteration, n_splits, scoring_function_to_use_for_evaluation, output_folder, cpus_per_job, gpu_per_job):

    ray.init(address='auto', _redis_password='5241590000000000', ignore_reinit_error=True, local_mode=False)
    # hyper_parameters = create_grid_parameters(hyper_parameters)
    analysis = hyperband_search_algorithm(training_dataset, oos_dataset, selected_features, model_function, hyper_parameters, n_iteration, n_splits, scoring_functions, scoring_function_to_use_for_evaluation, output_folder, cpus_per_job, gpu_per_job)

    # Get a dataframe for analyzing trial results.
    results_path = Path(output_folder) / Path("hyperband_optimization_results.csv")
