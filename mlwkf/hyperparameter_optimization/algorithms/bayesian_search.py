...
def bayesian_search_algorithm(training_dataset, oos_dataset, selected_features, model_function, hyper_parameters, scoring_functions, n_iteration, n_splits, scoring_function_to_use_for_evaluation, output_folder, cpus_per_job, gpu_per_job):
    ...
    bayesopt = ConcurrencyLimiter(bayesopt, max_concurrent=20)
    ...
    analysis = tune.run(
        ...
        name="bayesian_optimization_results",
...
def run_bayesian_search_algorithm(training_dataset, oos_dataset, selected_features, model_function, hyper_parameters, scoring_functions, n_iteration, n_splits, scoring_function_to_use_for_evaluation, output_folder, cpus_per_job, gpu_per_job):

    ray.init(address='auto', _redis_password='5241590000000000', ignore_reinit_error=True, local_mode=False)

    analysis = bayesian_search_algorithm(training_dataset, oos_dataset, selected_features, model_function, hyper_parameters, n_iteration, n_splits, scoring_functions, scoring_function_to_use_for_evaluation, output_folder, cpus_per_job, gpu_per_job)

    # Get a dataframe for analyzing trial results.
    results_path = Path(output_folder) / Path("bayesian_optimization_results.csv")
